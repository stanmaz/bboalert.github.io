...
function getClipboardData(newData) {
	navigator.clipboard.readText().then((cbd) => {
		...
		if (getDataType(cbData) == 'BBOalert') {} else {
			...
			for (let i = 0; i < alertTable.length; i++) {
...
function execUserScript(txt) {
	...
	for (var i = 0; i < rec.length; i++) {
		if (i % 2 == 0) {
			...
			if (script != '') {
				txt1 = txt1 + userScript(rec[i], script, foundContext, getContext(), foundCall, callText);
			} else {
				txt1 = txt1 + "%" + rec[i];
...
function getAlert() {
	...
	var elAlertExplain = getExplainInput();
	if (elAlertExplain == null) return;
	execUserScript("%onFindAlertStart%");
	BBOalertEvents().dispatchEvent(E_onFindAlertStart);
	var alertText = findAlert(getContext(), callText);
	execUserScript("%onFindAlertEnd%");
	BBOalertEvents().dispatchEvent(E_onFindAlertEnd);
